...
void ASpawnerVolume::AddObjects(int Amount)
{
	...
	for (int i = 0; i < SpawnPoints.Num(); i++)
	{
		...
		default:
			{
				APawn* NewAI = SpawnAIEntity(SpawnPoint, ActorRotation);
				if (NewAI)	SpawnedAI.Add(NewAI);
			} break;
		}
	}
}
...
void ASpawnerVolume::DeleteAllObjects()
{
	for (int i = 0; i < SpawnedActors.Num(); i++)
	{
		if (SpawnedActors[i])	{ SpawnedActors[i]->Destroy(); }
	}

	for (int i = 0; i < InstancedMeshes.Num(); i++)
	{
		if (InstancedMeshes[i])	{ InstancedMeshes[i]->Destroy(); }
	}

	for (int i = 0; i < SpawnedAI.Num(); i++)
	{
		if (SpawnedAI[i])	{ SpawnedAI[i]->Destroy(); }
	}
